...
export default function Thread({ state, services, ...props }) {
	...
	return (
		<ThreadContainerStyled $maxwidth={maxwidth} $maxheight={maxheight} $variant={variant} $highlighted={highlighted} className={'thread-container'} onClick={() => threadListener()}{...props}>
			<IconContainer>
				<FiMessageSquare id={`thread-message-icon-${idno}`} />
				<p id={`thread-name-${idno}`}>{name}</p>
				<FiTrash2 id={`thread-trash-icon-${idno}`} className='last-icon' onClick={e => { e.stopPropagation(); trashListener() }} />
			</IconContainer>
