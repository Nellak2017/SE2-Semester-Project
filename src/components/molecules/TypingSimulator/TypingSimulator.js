...
const TypingSimulator = ({ message, typingSpeed }) => {
	...
	const typeCharacter = useCallback(currentIndex => {
		if (currentIndex >= message?.length) return
		setTimeout(() => { setTypedText(prev => prev + message[currentIndex]); typeCharacter(currentIndex + 1) }, calculateDelay())
	}, [message, calculateDelay])
	useEffect(() => { if (initialMount.current) { typeCharacter(0); initialMount.current = false } }, [message, typingSpeed, typeCharacter])
